...
//输就翻倍，赢收场
@Component
public class BetForSMPSafeOperation {

    private static final Logger logger = LoggerFactory.getLogger(BetForSMPSafeOperation.class);
    ...
    @Autowired
    public BetForSMPSafeOperation(SMPBetRepository smpBetRepository,
...
//                loser.  不加筹码
    ...
    private String getInputName(SingleBetCategory category, int position) {
        ...
        switch (category) {
            case DA:
                name = String.format("DX%s_D", position);
                break;
            case XIAO:
                name = String.format("DX%s_X", position);
                break;
            case DAN:
                name = String.format("DS%s_D", position);
                break;
            case SHUANG:
                name = String.format("DS%s_S", position);
                break;
            case LON:
                name = String.format("LH%s_L", position);
                break;
            case HU:
                name = String.format("LH%s_H", position);
                break;
        }
