...
//        js.executeScript("window.scrollBy(0, 1500);","");
        ...
        while (retrySubmit) {
            try {
                WebElement submitBtn = DriverUtils.returnOnFindingElement(driver, By.cssSelector("input[value=确定]"));
                Thread.sleep(1000);
                submitBtn.click();
                retrySubmit = false;
            } catch (Exception e) {
        ...
        DriverUtils.returnOnFinishLoadingGivenFinishingIndicator(driver, "请确认注单");
        WebElement form = DriverUtils.returnOnFindingElement(driver, By.cssSelector("div[aria-describedby=betsBox]"));
        try {
            if (isMimic) {
                Thread.sleep(4000);
                logger.info("[Operation - Finish Bet] 模拟下注");
                List<WebElement> aList = form.findElements(By.tagName("span"));
                ...
                Thread.sleep(1000);
                form = DriverUtils.returnOnFindingElement(driver, By.cssSelector("div[aria-describedby=messageBox]"));
                aList = form.findElements(By.tagName("span"));
                for (WebElement a : aList) {
